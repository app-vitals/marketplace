...
Read `state/todos.json`. Find `shipwright-lite` tasks where:
- `status` is `pending`
...
2. Read the files listed in the task's `description` that are relevant to the implementation
3. Read nearby test files to understand the testing patterns in use
...
Run the project's validation commands. Detect them from `package.json` scripts or equivalent:
...
```bash
# Write body to /tmp/slite-pr-{task-id}.txt first, then:
gh pr create \
  --repo app-vitals-com/{repo} \
  --title "feat({id}): {task title}" \
  --body-file /tmp/slite-pr-{task-id}.txt
rm /tmp/slite-pr-{task-id}.txt
```
