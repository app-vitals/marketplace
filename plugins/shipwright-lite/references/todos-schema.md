# Todos Schema — Shipwright Lite Tasks

Shipwright Lite tasks live in `state/todos.json` alongside existing workspace todos.
The `source: "shipwright-lite"` field distinguishes them from `eng-execute` tasks.
...
| `source` | `"shipwright-lite"` | Distinguishes from `eng-execute` tasks |
...
Blocked tasks surface in the morning brief and require human intervention before the execution cron will pick them up again.

## Coexistence with eng-execute

Eng-execute tasks use `source: "eng-execute"` (or have no `source` field for legacy entries). The execution cron filters by `source: "shipwright-lite"` so there is no conflict.
