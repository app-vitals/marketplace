...
Blocks reads to zero-access files via PreToolUse hook on Read tool.
...
def get_config_path() -> Path:
    ...
    # 1. Check project hooks directory (project-specific override)
...
def check_path(file_path: str, config: Dict[str, Any]) -> Tuple[bool, str]:
    """Check if file_path is blocked from reading. Returns (blocked, reason).

    Only zero-access paths block reads. Read-only and no-delete paths allow reads.
    """
    # Check zero-access paths (no operations allowed, including reads)
    for zero_path in config.get("zeroAccessPaths", []):
        if match_path(file_path, zero_path):
            return True, f"zero-access path {zero_path} (no operations allowed, including reads)"
...
def main() -> None:
    ...
    if blocked:
        print(f"SECURITY: Blocked read to {reason}: {file_path}", file=sys.stderr)
        sys.exit(2)
