...
def check_command(command: str, config: Dict[str, Any]) -> Tuple[bool, bool, str]:
    ...
    for zero_path in zero_access_paths:
        if is_glob_pattern(zero_path):
            ...
            try:
                # Add path boundaries to prevent matching inside words
                # e.g., "*.env" shouldn't match "environment"
                bounded_regex = r'(?:^|[\s=/])' + glob_regex + r'(?:[\s/]|$)'
                if re.search(bounded_regex, command, re.IGNORECASE):
                    return True, False, f"Blocked: zero-access pattern {zero_path} (no operations allowed)"
            except re.error:
                continue
        else:
            # Original literal path matching with path boundaries
            # Use lookahead/lookbehind to ensure we match complete path components
            # This prevents ".env" from matching inside "environment"
            expanded = os.path.expanduser(zero_path)
            escaped_expanded = re.escape(expanded)
            escaped_original = re.escape(zero_path)

            # Add word/path boundaries: must be preceded/followed by space, slash, start/end
            pattern_expanded = r'(?:^|[\s=/])' + escaped_expanded + r'(?:[\s/]|$)'
            pattern_original = r'(?:^|[\s=/])' + escaped_original + r'(?:[\s/]|$)'
